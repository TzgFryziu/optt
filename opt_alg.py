...
def fib(ff, a, b, epsilon):
    ...
    for i in range(k - 2):
        if ff(c[i]) < ff(d[i]):
            ...
            b.append(d[i])
            d.append(c[i])
            c.append(b[i + 1] - fib_seq(k - i - 3) / fib_seq(k - i - 2) * (b[i + 1] - a[i + 1]))
        else:
            a.append(c[i])
            b.append(b[i])
            c.append(d[i])
            d.append(a[i + 1] + fib_seq(k - i - 3) / fib_seq(k - i - 2) * (b[i + 1] - a[i + 1]))
        i+=2
...
def lag(ff, a, b,c, epsilon, gamma, nmax):
    i = 0
    f = 0
    a = [a]
    b = [b]
    ...
    while True:
        ...
        m = (ff(a[i]) * (b[i] - c[i]) +
             ...
             ff(c[i]) * (a[i] - b[i]))
        f+=6
        if m == 0:  # Handling the division by zero scenario
        ...
        # Check if d[i] is within bounds and update points accordingly
        if a[i] < d_new < c[i]:
            f+=2
            if ff(d_new) < ff(c[i]):
                a.append(a[i])
        ...
        # Stopping conditions based on interval size or successive d values
        if (b[i] - a[i]) < epsilon:
            break
        if len(d) > 1 and abs(d[-1] - d[-2]) < gamma:
            break

    return d[-1],ff(d[-1]),f  # Return the last d value, which is the approximate solution
...
def lag_r(ff, a, b, epsilon, gamma, nmax, params):
    ...
    while True:
        # Calculate the numerator (l) and denominator (m) for d[i]
        l = (ff(a[i], params) * (b[i]**2 - c[i]**2) +
             ff(b[i], params) * (c[i]**2 - a[i]**2) +
             ff(c[i], params) * (a[i]**2 - b[i]**2))

        m = (ff(a[i], params) * (b[i] - c[i]) +
